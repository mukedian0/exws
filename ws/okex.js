...
class WSv1 extends EventEmitter {
    ...
    _handleChannel(msg) {
        try {
            ...
            const channelData = msg.shift() // Pop the first element

            if (channelData.channel === 'addChannel') {
                debug('Subscription report received')
                this._channelMap[channelData.data.channel].connect = true;
                return;
            }

            const event = this._channelMap[channelData.channel]
            ...
            if (event) {
                ...
                if (event.channel === 'book') {
                    this._processBookEvent(channelData.data, event)
